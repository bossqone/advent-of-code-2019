...
abstract class Day(day: Int) extends App {
  ...
  private def readInput(): String =
    Source.fromFile(s"input/$formatted.txt").getLines().mkString(lineSeparator)
  ...
  protected val lines: IndexedSeq[String] =
    input.split(lineSeparator).toIndexedSeq
